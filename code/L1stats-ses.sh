...
if [ ! -e $DATA ]; then
	echo "missing data sub-${sub} ses-${ses}: $DATA "		
...
if [ ! -e $CONFOUNDEVS ]; then
	echo "missing confounds sub-${sub} ses-${ses}: $CONFOUNDEVS "
	echo "missing confounds: $CONFOUNDEVS " >> ${maindir}/re-runL1.log
	exit # exiting to ensure nothing gets run without confounds
fi

# Specify EVs
EVDIR=${maindir}/derivatives/fsl/EVfiles/sub-${sub}/ses-${ses}/${TASK}/
...
if [ "$ppi" == "ecn" -o  "$ppi" == "dmn" ]; then
	...
	OTEMPLATE=${MAINOUTPUT}/L1_sub-${sub}_${ses}_task-${TASK}_model-1_seed-${ppi}_run-${run}.fsf
	...
	if [ "$ppi" == "0" ]; then
		...
		OUTPUT=${MAINOUTPUT}/L1_task-${TASK}_ses-${ses}_model-1_type-${TYPE}_run-${run}_sm-${sm}
	else
		TYPE=ppi
		#OUTPUT=${MAINOUTPUT}/L1_task-${TASK}_model-1_type-${TYPE}_seed-${ppi}_run-${run}_sm-${sm}
		OUTPUT=${MAINOUTPUT}/L1_task-${TASK}_ses-${ses}_model-1_type-${TYPE}_seed-${ppi}_run-${run}_sm-${sm}
	fi

	# check for output and skip existing
	if [ -e ${OUTPUT}.feat/cluster_mask_zstat1.nii.gz ]; then
		echo "Skipping sub-${sub}, ses-${ses}, task-${TASK}, output already exists"		
		exit
	else
