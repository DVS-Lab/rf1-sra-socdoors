...
# Combine one participant's Social Doors and Doors L1 estimates in FEAT.
...
TYPE is act, ppi_seed-<seed>, or nppi-<network>.

Options:
  --session ID    BIDS session (default: 01)
  --dry-run       Validate and print paths without writing
  --render-only   Render the .fsf but do not run FEAT
...
case "$type" in
    act) template_type="act"; ncopes=4 ;;
    ppi_seed-*) template_type="ppi"; ncopes=5 ;;
    nppi-*) template_type="nppi"; ncopes=5 ;;
    *) echo "ERROR: unsupported L2 type: $type" >&2; exit 2 ;;
esac

smoothing=5
input1="$(l1_output_base "$sub" "$session" socialdoors 1 "$type" "$smoothing").feat"
input2="$(l1_output_base "$sub" "$session" doors 1 "$type" "$smoothing").feat"
for input in "$input1" "$input2"; do
    [[ -d "$input" ]] || { echo "ERROR: required L1 input not found: $input" >&2; exit 1; }
done

output="$(l2_output_base "$sub" "$session" "$type" "$smoothing")"
template="${PROJECT_ROOT}/templates/L2_task-socialdoors_model-1_type-${template_type}.fsf"
subject_output="${FSL_DERIVATIVES_ROOT}/sub-${sub}/ses-${session}"
rendered="${subject_output}/L2_sub-${sub}_task-socialdoors_ses-${session}_model-1_type-${type}.fsf"
[[ -f "$template" ]] || { echo "ERROR: FEAT template not found: $template" >&2; exit 1; }

printf 'L2 plan (within-participant social + monetary comparison)\n  Social Doors: %s\n  Doors: %s\n  output: %s.gfeat\n  FSLSUB_PARALLEL: %s\n' \
