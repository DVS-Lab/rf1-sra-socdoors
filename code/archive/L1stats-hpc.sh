...
#PBS -N L1stats-socdoors_srndna-pilot
#PBS -q normal
#PBS -m ae
#PBS -M james.wyngaarden@temple.edu
...
projectdir=$shareddir/rf1-sra-socdoors
scriptdir=$projectdir/code
bidsdir=$rf1datadir/bids
logdir=$projectdir/logs
mkdir -p $logdir

rm -f L1stats-doors*.o*
rm -f L1stats-doors*.e*≈
...
for sub in ${subjects[@]}; do
	for run in 1; do
		...
		SHAPE_MISSED_TRIAL=${EVDIR}_decision-missed.txt
		if [ -e $SHAPE_MISSED_TRIAL ]; then
			EV_SHAPE=3
		else
			EV_SHAPE=10
		fi
